#! /bin/bash                                                                                                        
# Batch submission job script, executes run_EIC_batch_5on41.csh script
...
while [[ $i -le $NumFiles ]]; do
    batch="${USER}_EICDempGen_5on41_${i}_Job.txt" # The name of the job submission script it'll create each time
    echo "Running ${batch} for file ${i}"
    cp /dev/null ${batch}
    RandomSeed=$(od -An -N3 -i /dev/random)
    echo "#!/bin/csh" >> ${batch} # Tells your job which shell to run in
    echo "#PBS -N DEMPGen_5on41_${NumEvents}_${i}" >> ${batch} # Name your job                     
    ...
    echo "#PBS -o  /home/${USER}/trq_output/DEMPGen_5on41_${NumEvents}_${i}.out" >> ${batch} # Output directory and file name, set to what you like
    echo "#PBS -e  /home/${USER}/trq_output/DEMPGen_5on41_${NumEvents}_${i}.err" >> ${batch} # Error output directory and file name
    echo "date" >> ${batch} 
    echo "cd /home/apps/DEMPgen/" >> ${batch} # Tell your job to go to the directory with the script you want to run
    echo "./run_EIC_batch_5on41.csh ${i} ${NumEvents} ${RandomSeed} ${OutputType}" >> ${batch} # Run your script, change this to what you like
    echo "date">>${batch}
